!function() {
    ...
    var templateStr = '<div class="ui-dialog dialog-danger modal-bssdanger js-bz-div">\
    	<div class="modal-header">\
    	<span class="modal-title"><i class="glyphicon glyphicon-remove-circle margin-right-sm"></i>{{title}}</span>\
    	</div>\
    	<div class="modal-body">\
    	<h2 class="text-danger">{{messageH}}</h2>\
    	<h3>{{{message}}}</h3>\
    	<p>{{date}}</p>\
...
!function() {
	...
	$.widget("ui.bssfileshow", {
        ...
        _create:function(){
        	...
        	this.$fileMsg = this.element.find(".js-file-msg");
        	this.$fileImage = this.element.find(".js-file-image");
        	this.$fileName = this.element.find(".js-file-name");
        	this.$fileSize = this.element.find(".js-file-size");
        	this.$fileProcess = this.element.find(".js-file-process");


        	this.$fileOpt = this.element.find(".js-file-opt");
        	this.$fileDown = this.element.find(".js-file-down");
        	this.$fileRead = this.element.find(".js-file-read");
        	this.$fileDel = this.element.find(".js-file-del");
        ...
        _init:function(){
        	...
        	this.$fileName.html(this.options.name);
        	 var filesize = (this.options.size/Math.pow(1024,2)).toFixed(2)+'MB';
         	this.$fileSize.html(filesize);
...
!function(){
	...
	$.widget('ui.bssmenunav',{
		options:{
            ...
            navplace:'left' //导航栏位置，默认在左边，也可以在右边
		},
		_init:function(){
			var options = this.options;
			if(!options.navWin && !options.navwinArr){
				...
				this.$splitterWin = options.splitterWin ? options.splitterWin : this.$win.parent();
		        ...
		        this.dHeight = options.scrolWin ? $(options.scrolWin).height() : $(document).height();
		        this._make();
		        this._bindEvent();
		 ...
		 */
		_make:function(){
			var NavHtml = '<div class="step-heading ui-pane-heading"></div>\
				 			<div class="step-operation ui-pane-control">\
					 		...
					 		<div class="step-list ui-pane-body"><ul class="js-step"></ul></div>\
						 		<div class="ui-pane-footer">\
            ...
            fish.each($hs,function(hs,index){
	 		...
	 		this.$pageNavUl = this.$pageNavList.find("ul.js-step");
            this.$pageNavUl.html(temp.join(""));
            ...
            var _panel = [
				            { collapsible: false, size: "200px",scrollable: true },
				            { collapsible: false }
				        ];
            if(this.options.navplace == 'right'){
            	 _panel = [
				            { collapsible: false},
				            { collapsible: false, size: "200px",scrollable: true  }
				        ];
            }
            this.$splitterWin.splitter({
				 panes: _panel,
				 afterResize:function(){
					  fish.resize($(".ui-jqgrid",This.$win),true);
				 }
			});
        ...
        _scrollEventHandler:function(event){
        	$(".fixedBox:visible").hide() //滚动的时候隐藏所有显示的浮动框
...
//            var $this = $(window);
            var $this = This.options.targetWin ? This.options.targetWin : $(window);
            clearTimeout(timer);
            timer = setTimeout(function(e){
                This.scrollIng = true;
                This.scrollToBottom = $this.scrollTop()+1 + $this.height()  >= This.dHeight;
                //加载未加载视图
...
//            });
            ...
            if(This.options.targetWin){
              This.options.targetWin.animate({
                "scrollTop": offset
            }, this.options.scrollSpeed, this.options.easing, function(){
                This.scrollIng = false;
            });
...
!function() {
	...
	$.widget("ui.slidingdoor", {
		...
		_bodyEvent: function (e) {
			...
			if (that.hasExit && that.hasBidingBodyEvent && $('body>.ui-dialog').length==0 ){   // 判断页面没有弹出框存在

				//在推拉门之外点击
				if( $(e.target).parents('.js-slidingDoorDiv').length == 0 && !$(e.target).hasClass("js-slidingDoorDiv") ){
					if(e.target.className != 'glyphicon glyphicon-remove'){  //判断事件源是否为弹出窗的关闭按钮
						that.hide();
					}
				}else{
					//推拉门范围之内的关闭X
					if(e.target.className.indexOf('sliding-panel-close') >= 0 || e.target.className.indexOf('SLIDING-PANEL-CLOSE') >= 0){
						that.hide();
					}
				}
			}
	...
	$.widget('ui.combobox',$.ui.combobox,{
		...
		_initDataSource:function(params,defer){
			...
			if(fish.isObject(params)){
				...
				params.attrCode = params.attr_code;
			}else{
				attr_code = params;
			}
			...
			if(attr_code === null || attr_code === "" ){
				...
				if(!fish.isObject(params) && fish._codecache[attr_code]){
					...
					that._trigger('dataset',that.options.dataSource);
					if(defer) {
						defer.resolve();
					}else{
						that._superInit();
		...
		render: function(items, query) {
            var that = this,name = this.options.dataTextField;

            items = $(items).map(function (i, item) {

            	if(that.options.sub_data === 'parent' || !that.p_attr_value || that.p_attr_value === item.parentValueId || item.attrValue === "-1"){
            		 var title = item[name];
            		 if(that.options.dataTitleField && item[that.options.dataTitleField]){
            			 title = item[that.options.dataTitleField];
            		 }
            		 var li = $(MENU_ITEM).data('value', item).attr('title', title);
            		 li.html(that.highlighter(item[that.options.dataTextField], query));


                     if (that._selectedItem) {
                         if (that._selectedItem[name] === item[name]) {
                        	 li.addClass('active');
                         }
                     } else if (i === 0) {
                    	 li.addClass('active');
            ...
            if (this.options.template) {
                this.$menu.append(this.options.template);
            }
            return this;
        },
        /**
...
! function(){
	...
	$.widget('ui.combogrid',$.ui.combogrid,{
        _create: function () {
            var self = this;
            /*self.element.attr('placeholder', self.options.placeholder);
            // 使用placeholder插件，兼容IE
...
!function(){
	...
	$.widget('ui.fixedbox',{
		options:{
			...
			content: "",//内容，接受HTML字符串或jquery对象
			template: '<div class="fixedBox" role="fixedBox"></div>',
			addClass:'',//设置高度的class->fixedBox-middle
		...
		_init:function(){
			...
			if(this.options.addClass==true)
			{
				this.$fixedbox.addClass('fixedBox-middle');
			}
			else if(typeof(this.options.addClass)=='string')
			{
				this.$fixedbox.addClass(this.options.addClass);
			}
...
//			}
		...
		hide:function(e){
			...
			if(e){
				...
				if(!$(e.target).attr("ui-fixedbox")){
					var temp_parent = $(e.target).parents("[ui-fixedbox]");
					if(temp_parent.length > 0){
						if(temp_parent[0].className === element.className){
							return;
						}
					}
				}
...
! function(){
	...
	$.widget('ui.grid',$.ui.grid,{
		options:{
			...
			searchbtn:false, //搜索栏是否显示搜索按钮，默认为false
			allselarrrow:[], //服务端分页，勾选的rowid列表
			allselarrrowData:[], //服务端分页，勾选的rowData列表
            relativeDom: {
        		...
        		ts.options.pageData = (function(){
        			return function(page, rowNum, sortname, sortorder,filterCol, filterVal){
        				...
        				fish.post(origUrl,p_param,function(reply){
        					//1.reload数据之前，把之前的选中数据保存起来
        					if(ts.options.multiselect){
        						//ts.options.allselarrrow = _.union(ts.options.allselarrrow,ts.options.selarrrow);
    			        		//ts.options.allselarrrowData = _.union(ts.options.allselarrrowData,ts.getCheckRows());
    			        		ts._unionSelrowData();
        					}

			        		//2.reload数据之后,selarrrow = [];
			        		ts.reloadData(reply);

			        		//3.恢复当前页勾选情况 selarrrow = allselarrrow
			        		if(ts.options.multiselect){
			        			ts.setCheckRows(ts.options.allselarrrow, true);
			        		}
                		});
        ...
        _unionSelrowData:function(){
        	var ts = this;
        	//当前页数据
        	var _data = this.options.data;
        	//去掉当前页所有数据
        	var _allselarrrow = [] , _allselarrrowData = [];
        	var keyName = ts.options.keyName ? ts.options.keyName : 'id';
        	for(var i=0;i<ts.options.allselarrrow.length;i++){
        		var in_data = false;
        		for(var j=0;j<_data.length;j++){
            		if(_data[j][keyName]+"" === ts.options.allselarrrow[i]){
            			in_data = true;
            			break;
            		}
            	}
        		if(!in_data) {
        			_allselarrrow.push(ts.options.allselarrrow[i]);
        		}
        	}
        	//根据_allselarrrow 得到 _allselarrrowData
        	ts.options.allselarrrowData.map(function(item){
        		if( item && _.contains(_allselarrrow, item[keyName]+"") ){
        			_allselarrrowData.push(item);
        		}
        	});

        	//再加上当前页勾选的数据
        	ts.options.allselarrrow = _.union(_allselarrrow,ts.options.selarrrow);
        	ts.options.allselarrrowData = _.union(_allselarrrowData,ts.getCheckRows());
        },

        //服务端分页，多选模式下面 获取所有勾选的数据
        getAllCheckRowss: function () {
        	this._unionSelrowData();
            return this.options.allselarrrowData;
        },

        clearAllCheckRowss:function(){
        	this.options.allselarrrow = [];
        	this.options.allselarrrowData = [];
        },
        //重写父类方法，去掉undifine元素
        getCheckRows: function () {
            ...
            for (var i = 0; i < selarrrow.length; i++) {
            	if(ts.p.data[ts.p._index[selarrrow[i]]])
        ...
        exportData:function(param,isCurrentPage){
        	...
        	var exportData = {
                  "param": param,
                  "filename": param.filename || "excel.xls" //导出的文件名
            };
            ...
            if (isCurrentPage) {
            	 var _data = this.options.data;
            	 if(this.options.datatype === "local" ){ //如果是客户端分页，取当前页数据
            		 var currentPageIndex = $(this.element).grid("getGridParam","page"),
                     currentRowCount = $(this.element).grid("getGridParam","rowNum");
            		 _data = this.options.data.slice((currentPageIndex - 1) * currentRowCount,
                                    currentPageIndex * currentRowCount);
            	 }
            	_data = _data.map(function(item) {// 处理隐藏列
                                return _.pick(item, function (value, key, object) {
                                    return !(_.contains(_hiddens, key));
                                }, _hiddens)
                            }).map(function(item) { // 处理attr_code翻译
                                return _.object(_.keys(item),
                                                _.map(item, function (value, key, object) {
                                                    var result = value || "",
                                                        attr_code;
                                                    var col = _.find(_cols, function (item) {
                                                        return item.name == key;
                                                    }, key);
                                                    if (col && col.attr_code) {
                                                        attr_code = col.attr_code;
                                                    } else {
                                                        return value;
                                                    }
                                                    if(fish._codecache[attr_code]){
                                                        _.each(fish._codecache[attr_code],function(data){
                                                            if(data.attrValue == value && data.attrValueDesc){
                                                                result = data.attrValueDesc;
                                                                return;
                                                            }
                                                        }, value);
        ...
        addTreeLine: function () {
        	...
        	_.each(trArr, function (item, index, list) {
        		...
        		//获取下一个节点的data-level
        		var nextLevel = $(trArr[index+1]).attr("data-level");
        		...
        		if (dataLevel != undefined) {
            		...
            		if (dataLevel == currentSiblingDataLevel) {
            			...
            			if (dataLevel != 0) { //根节点不修改任何样式
            				...
            				//若下一个层级比当前层级大，证明当前层级是父节点，有图标
            				if( nextLevel > dataLevel ) {
            					tree_wrap.css({
...
//            					$(td).prepend('<span class="ui-treeline-end"></span>');
            					...
            					$(td).children("span.ui-treeline-first").remove();
            					$(td).children("span.ui-treeline-center").remove();
            			...
            			if (dataLevel != 0) { //根节点不修改任何样式
            				...
            				if (dataLevel != 1) {
            					...
            					_.times(currentSiblingDataLevel - 1, function () {
...
//                					$(parent_td).prepend('<span class="ui-treeline-center"></span>');
            		...
            		} else {    //dataLevel < currentSiblingDataLevel
            			//弹出当前元素的层级与上一个元素层级相差的数量
            			var popTimes = currentSiblingDataLevel - dataLevel;
            			_.times( popTimes, function() {
            				stack.pop();
            			})
        				...
        				if (index == trArr.length - 1) {
        					$(td).children("span.ui-treeline-first").remove();
        					$(td).children("span.ui-treeline-center").remove();
        					$(td).children("span.ui-treeline-end").remove();
...
!function () {
    ...
    $.extend($.ui.grid.prototype, {
        ...
        searchGrid: function () {
            ...
            searchOpts = ts.p.searchMode === "filter" ? {target: ts.element, autocomplete: false,searchbtn:this.options.searchbtn}
                : {target: ts.element,searchbtn:this.options.searchbtn};
            ...
            function scrGrid(iR) {
                ...
                if (rpos < 0 || (rpos + rh) > ch) {
                    ts.$slimScroll.slimscroll({
                        scrollToY: ts.$slimScroll.scrollTop() + rpos + rh - ch
                    });
           ...
           var scb = e != null && e.target != null && $(e.target).hasClass("cbox");
            if (ts.p.multiselect && scb) {
                ...
                if (ia === -1) {
                    ...
                    $("[id='jqg_" + ts.p.id + "_" + pt.id + "']", ts.element).prop("checked", true);
                } else {
                    ts.p.selarrrow.splice(ia, 1);
                    $("[id='jqg_" + ts.p.id + "_" + pt.id + "']", ts.element).prop("checked", false);
                }
            }
            var ia = false;
            if(e&&e.target&&e.target.className&&e.target.className=="cbox")
             {
                 ia = $(e.target).is(":checked");
             }
            else
             {
            	if(e&&e.target)
                 ia = $(e.target).parent("tr").find('.cbox').is(":checked")
             }
            // ia = $.inArray(ts.p.selrow, p);
            ia === false ? checked = false : checked = true;

            if (ts.p.treeGrid) { //自己如果没有显示,则显示出来
                ts._setVisibleNode(pt.id);
                if (ts.p.multiselect && scb) {
                    ts._cascadeCheckedNew(checked);
                }
            }
        ...
        //覆盖fish_cascadeChecked方法 判断disabled的节点不勾选
        _cascadeCheckedNew : function (stat) {
            var ts = this;
            ...
            if (ts.p.checkChildren) {
                ...
                $(result).each(function (i) {
                    ...
                    if(!$("#jqg_" + ts.p.id + "_" + rowid, ts.element).prop("disabled")){
            ...
            if (ts.p.checkParent) {
                ...
                $(result).each(function (i) {
                    ...
                    ia = $.inArray(rowid, ts.p.selarrrow);
                    if (ia === -1 && stat ) {
                        if(!$(document.getElementById("jqg_" + ts.p.id + "_" + rowid)).prop("disabled")){
...
!function(){
	...
	$.widget('ui.hypercombotree',$.ui.combotree,{
		...
		_create:function(){
			...
			this.$searchMain = $template.find(".search-main");
			//输入判断用函数
...
//					}
					//将选中的项目滚动到成为第一项的位置
					ts.$searchMain.scrollTop(li_dom.position().top);
					if(double_click){
						//如果是双击，那么就勾选它
    ...
    $.widget("ui.label",$.ui.label,{
        ...
        _create : function () {
            ...
            if(typeof width == 'number'){
                $($(this._contentElement).find(".label-title")[0]).css("max-width",width);
            }
...
!function () {
	...
	$.widget("ui.multiselect",$.ui.multiselect,{
		_delegateEvent:function(){
			this._super();
			var that = this;
			if(that.options.itemClick){
				that._on(that.$ul, {
	                'click .overflow-ellipsis': function itemClick(evt) {
	                    evt.preventDefault();
	                    evt.stopPropagation();
	                    that._trigger("itemClick",null,$(evt.target).text());
	                }
	            });
			}
		},
            ...
            switch (stroke) {
                case KEY_CODE.BACKSPACE:
                    ...
                    if (!this.$input.val() && this._choicesCount() > 0) {
                        ...
                        this._clearHighlightMenu();
                        return this._search(this.$input.val().slice(0, -1));
                    }
                    break;
                case KEY_CODE.TAB:
                    //TAB
                ...
                case KEY_CODE.ENTER:
                    //ENTER
                    evt.preventDefault();
                    if(this.options.getSelectedItem)//判断传参
                    {
                    	var high,itemId;
                    	if (this.result_highlight) {
                    		 high = this.result_highlight; //获取焦点Li元素
                    		 this._clearHighlightMenu(); //清除menu上焦点
                    		 itemId = this.results_data[high[0].getAttribute("data-option-array-index")];//获取该元素数据
                    	 return this.options.getSelectedItem(itemId);
                    	}
                    }
                ...
                case KEY_CODE.DOWN:
                    ...
                    break;
                default:
                    return this._search(this.$input.val() + evt.key);
            }
        },
        /**
         * @method data
        ...
        _setDataSource: function(data, value, name) {
            ...
            if ($.type(data) === 'array') {
                ...
                $.each(data, function (index, obj) {
                	var dname = fish.escape(obj[name]);
                	var dtitle = obj['title'] ? fish.escape(obj['title']) : dname;
                	opts += '<option value="' + obj[value] + '"' + 'title="' + dtitle + '"' + (obj.disabled ? ' disabled="true" ' : '') + '>' + dname + '</option>';
                });
                this.element.empty().html(opts);
                this._update();
            }
        },
        _choiceBuild: function _choiceBuild(item) {
            var choice, closeBtn;

            var html = "<span>" + item.html + "</span>";
            if (this.options.optionTruncate) {
                html = "<span class='overflow-ellipsis'>" + item.html + "</span>";
            }

            choice = $('<li />', {"class": "search-choice"}).html(html);
            var title =  $(this.element).find("option[value='"+item.value+"']").attr("title");
            title = title ? title :item.html;
            choice.find('span').attr('title', title);

            if (item.disabled) {
                choice.addClass('search-choice-disabled');
            } else {
                closeBtn = $('<button class="close" type="button"><span title="close">&times;</span></button>');
...
!function () {
    ...
    var modalStack = {
        open: function (modalInstance, options) {
            ...
            //如果存在收缩的弹框，不允许再弹出窗口，避免乱搞 暂时这样
        	if($(".ui-fold-dialog").length > 0 && !$(".modal-backdrop.in").is(":visible")){
        		if($modalElement[0].className.indexOf('modal-') < 0){ //屏蔽info warn confimr这些
        			return false
            	}
        	}
            ...
            //增加弹框的收缩，展开功能
            if(options.canFold === true ){
            	if($modalElement[0].className.indexOf('modal-') < 0){ //屏蔽info warn confimr这些
                	if ($modalHeader.find('.modal-mini').length === 0) {
                        $modalHeader.prepend('<button class="modal-mini data-min"><span class="glyphicon glyphicon-minus"></span></button>');
                    }

                	if ($modalHeader.find('.modal-maxi').length === 0) {
                        $modalHeader.prepend('<button class="modal-maxi data-max"><span class="maxi-ico"></span></button>');
                    }
            	}
            ...
            $modalElement
                .on('click.dismiss.data-api', '[data-dismiss]', function (e) {
                	if(options.beforeClose && fish.isFunction(options.beforeClose)){
                ...
                .on('click','.data-max', function (e) {
                	//展开
                	 modalStack.max(modalInstance);
                })
            	.on('click','.data-min', function (e) {;
            		//收缩
            		 modalStack.min(modalInstance);
        ...
        max: function(modalInstance){
        	var modalWin = openedWindows.get(modalInstance);
            if (modalWin) {
            	var $modalElement = modalWin.value.$modalElement;
            	$modalElement.removeClass("ui-fold-dialog");
           	 	$(".modal-backdrop.in").show();
           	 	$modalElement.find(".data-min").show();
           	 	$modalElement.animate({'height':$modalElement.pheight,'width':$modalElement.pwidth,'left':$modalElement.pleft,'top':$modalElement.ptop});
            }
        },
        min: function(modalInstance){
        	 if($(".modal-backdrop.in").is(":visible")){ //有遮罩层
        		 //判断所有弹框都已经收缩了 才隐蔽遮罩
             ...
             if (modalWin) {
            	 var $modalElement = modalWin.value.$modalElement;
            	 $modalElement.addClass("ui-fold-dialog");
            	 $modalElement.find(".data-min").hide();
            	 //保存之前的高度 宽度 位置
            	 $modalElement.pheight = $modalElement.height();
            	 $modalElement.pwidth = $modalElement.width();
            	 $modalElement.pleft = $modalElement.offset().left;
            	 $modalElement.ptop = $modalElement.offset().top;

            	 $modalElement.animate({'height':'30px','width':'200px','left':8,'top':$(window).outerHeight() - 38});
            	 $modalElement.minState = true;
             }
        },
...
!function() {
	...
	$.widget('ui.printpage', {
		...
		_create : function(){
			...
			tempForm.action = portal.appGlobal.get("webroot")+"/"+this.options.web_root+"/print/index.jsp";
			tempForm.target = 'printWin';
			tempForm.style.display = "none";

			var ids = JSON.stringify(that.options.printdata);
			ids = ids.replace(/\s/g, "&nbsp;");
			ids = ids.replace(/[\r\n]/g, "");
			...
			that.tempForm = tempForm;
			that.onsubmitFunc = function(name) {
				var li_width = that.options.width || 1000;
				var li_height = that.options.height || 800;
				var li_top = window.screenTop;
				var li_left = window.screenLeft;
				var ls_windowinfo = "scrollbars=yes,dialogWidth: " + li_width
						+ "px, dialogHeight: " + li_height + "px, help: no,"
						+ "dialogTop:" + li_top + "px,dialogLeft:" + li_left
						+ "px";
				var $printWin = window.open("about:blank", name, ls_windowinfo);
				that.$printWin = $printWin;
			};
		...
		postUrl : function() {
			var that = this;
			that.onsubmitFunc && this.onsubmitFunc('printWin');
			that.tempForm.submit();
			that.tempForm.remove();

	        window.onmessage = function(e){
	             if(e.data && e.data == 'closed'){
	            	 that.options.close && that.options.close();
	             }
	        };
		}
	});
...
!function () {
    ...
    var template = '{{#if filterFields}}\
		    ...
		    {{#if searchbtn}}\
			    <div class="input-group-btn">\
					<button class="btn btn-default js-searchBtn" type="button">搜索</button>\
				</div>\
			{{/if}}';


    $.widget("ui.searchbar", $.ui.searchbar, {
    	 _create: function () {
             ...
             this.element.append(fish.compile(template)({ filterFields: this.filterFields,searchbtn : this.searchbtn }));
             this.$searchBtn = this.element.find('.js-searchBtn');

             if (!_.isEmpty(this.filterFields)) {
                 this.$filter = this.element.find('.js-filter');

                 if (this.$filter[0].scrollWidth > this.$filter[0].clientWidth) {
                     this.$filter.parent().attr('title', this.$filter.html());
                 }

                 this._filterField = this.filterFields[0];
                 this.$input = this.element.find('input').clearinput();

                 if (this.filterFields.length > 1) {
                     this.$filterList = this.element.find('.js-filter-menu');
                 }
                 this._setOption('autocomplete', this.options.autocomplete);
             }
         ...
         _delegateEvents: function () {
            this._super();
            this._on(this.$searchBtn,{
            	'click' : '_onBtnSearch'
            });
         },
         _onBtnSearch :function(e){
        	 var field = this._filterField,
             	value = this.$input.val();
        	 this._trigger('search', e, [field, value]);
         }
    });
}();
!function(){
    ...
    $.widget("ui.tabs", $.ui.tabs, {
        options: {
            templateArr: ["<li><a href='#{href}'><button type='button' class='ui-tabs-close close' role='button'><span aria-hidden='true' title='close'>&times;<span class='sr-only'>Close</span></span></button>#{label}</a></li>",
                "<li><a href='#{href}'>#{label}<button type='button' class='ui-tabs-close ui-tabs-main-close close' role='button'><span aria-hidden='true' title='close' class='fa fa-times-circle'></span></button></a></li>"
            ],
...
!function () {
    ...
    $.widget('ui.datetimepicker', $.ui.datetimepicker, {
        ...
        _create: function () {
            var self = this;
            //self.element.attr('placeholder', '格式：yyyy-MM-dd');
            // 使用placeholder插件，兼容IE
            self.element.placeholder();
            return self._super();
        },
